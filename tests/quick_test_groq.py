...
if not api_key:
    ...
    print("Add GROQ_API_KEY=your_key to your .env file")
    sys.exit(1)

# Create client directly (no config change needed for this test)
try:
    client = GroqClient(api_key=api_key, model="llama-3.3-70b-versatile")
    print(f"✓ GroqClient created: model={client.model}")
...
try:
    ...
    response = client.complete_json(
        'Return a JSON object with fields: message="Hello from Groq", status="ok"',
        temperature=0.3,
        max_tokens=100
    )
